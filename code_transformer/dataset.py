...
# $수정필. 현재 Tag는 KEEP(0)와 REPLACE(3)만 존재함.
# 이를 확장하여 operation_ids > [명령어 id, 대체토큰 id, 삽입 id]를
# 생성하도록 수정.
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 수정 작업 ID 생성
        operation_ids = []
        for i in range(min(len(input_ids), len(output_ids))):
            if input_ids[i] == output_ids[i]:
                operation_ids.append(0)  # KEEP
            elif output_ids[i] == 0:  # 패딩 토큰
                operation_ids.append(0)  # KEEP
            else:
                operation_ids.append(3)  # REPLACE
        ...
        if len(operation_ids) > self.max_length:
            operation_ids = operation_ids[:self.max_length]
        else:
            operation_ids = operation_ids + [0] * (self.max_length - len(operation_ids))
