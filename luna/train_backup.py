...
from transformer.luna.model import LunaTransformerEncoder
...
def train():
    ...
    # 손실 함수 하이퍼파라미터
    EDIT_WEIGHT = 1.5  # 수정된 토큰에 대한 가중치
    NORMAL_WEIGHT = 1.0  # 수정되지 않은 토큰에 대한 가중치
    MIN_EDIT_RATIO = 0.1  # 최소 수정 비율

    # 데이터셋 및 데이터로더 설정
    transformer_path = "/content/drive/Othercomputers/학교 컴/대학원 과목/[Coding]/transformer"

    train_path = os.path.join(transformer_path, 'TrainData/combined_dataset.json')
    val_path = os.path.join(transformer_path, 'ValidationData/combined_validation_dataset.json')

    # 토크나이저 초기화 (이미 내부에서 학습 또는 로드를 수행)
    ...
    # 손실 함수 및 옵티마이저 설정
    criterion = WeightedCELossForGEC(
        pad_token_id=PAD_TOKEN_ID,
        edit_weight=EDIT_WEIGHT,
        normal_weight=NORMAL_WEIGHT,
        min_edit_ratio=MIN_EDIT_RATIO
    )
